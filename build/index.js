...
const calendarService = new CalendarService(env.GOOGLE_CLIENT_ID, env.GOOGLE_CLIENT_SECRET, env.GOOGLE_REFRESH_TOKEN);
const spotifyService = new SpotifyService(env.SPOTIFY_CLIENT_ID, env.SPOTIFY_CLIENT_SECRET);
// Setup tool handlers
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "send-email":
                await emailService.sendEmail(args.to, args.subject, args.body, args.html || false);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "schedule-email":
                await emailService.scheduleEmail(args.to, args.subject, args.body, new Date(args.scheduleTime), args.html || false);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "list-calendar-events":
                const events = await calendarService.listEvents(args.timeMin ? new Date(args.timeMin) : new Date(), args.timeMax ? new Date(args.timeMax) : undefined, args.maxResults || 10);
                const eventList = events.map(event => `${event.summary} - ${event.start.toLocaleString()}`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "update-calendar-event":
                ...
                if (args.startTime)
                    updates.start = new Date(args.startTime);
                if (args.endTime)
                    updates.end = new Date(args.endTime);
                if (args.location)
                    updates.location = args.location;
                const updatedEvent = await calendarService.updateEvent(args.eventId, updates);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "get-song-recommendations":
                const recommendations = await spotifyService.getRecommendations({
                    ...
                    limit: args.limit || 20,
                    valence: args.targetValence,
                    energy: args.targetEnergy,
                    danceability: args.targetDanceability,
                });
                const songList = recommendations.map(track => `${track.name} by ${track.artists.join(', ')}`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "search-spotify":
                const results = await spotifyService.search(args.query, args.type, args.limit || 20);
                let resultText;
                if (args.type === 'track') {
                    resultText = results.map((track) => `${track.name} by ${track.artists.join(', ')}`).join('\n');
                }
                else if (args.type === 'artist') {
                    resultText = results.map((artist) => `${artist.name} (${artist.followers} followers)`).join('\n');
                }
                else {
                    resultText = results.map((item) => item.name).join('\n');
                }
                return {
                    content: [
                        {
            ...
            case "create-spotify-playlist":
                const playlist = await spotifyService.createPlaylist(args.name, args.description, args.public || false);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "get-user-playlists":
                const playlists = await spotifyService.getUserPlaylists(args.limit || 20);
                const playlistList = playlists.map(playlist => `${playlist.name} - ${playlist.trackCount} tracks`).join('\n');
                return {
                    content: [
                        {
                            type: "text",
